...
namespace APICinemaProjectV2.Controllers
{
    ...
    public class MerchandisesController : ControllerBase
    {
        ...
        [HttpGet("GetMerchandiseAndOrderByID/{id}")]
        public async Task<ActionResult<IEnumerable<Movie>>> GetMerchandiseAndOrderByID(int id)
        {
            try
            {
                var result = await context.GetMerchandiseAndOrderByID(id);
